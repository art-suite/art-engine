...
module.exports = createWithPostCreate class PagingScrollElementWip extends Element
  ...
  constructor: ->
    ...
    @_atEnd = false
    @_atStart = true
    @_scrollPositionRelativeToChildrenAlignment = false
    @_scrollContents =
  ...
  @concreteProperty
    ...
    scrollPosition:
      ...
      postSetter: (position) ->
        ...
        @onNextReady => @_updateAtStartAndAtEnd()
        @_setScrollContentsLocation position
  ...
  @getter
    ...
    boundedScrollPosition: -> bound @getMinScrollPosition(), @getScrollPosition(), @getMaxScrollPosition()
  ...
  @getter
    ...
    currentPagePosition:       -> @getMainCoordinate @_scrollContents.getPendingCurrentLocation()
    pagesBeforeBaselineSize:   -> @getMainCoordinate @_pagesBeforeBaselineWrapper.getPendingCurrentSize()
    pagesAfterBaselineSize:    -> @getMainCoordinate @_pagesAfterBaselineWrapper.getPendingCurrentSize()
  ...
  @getter
    contentsFitInWindow: -> @getTotalPageSize() <= @getWindowSize()

  _setScrollContentsLocation: (scrollPosition) ->
    if @getContentsFitInWindow()
      @_scrollContents.setAxis axis = @getPendingChildrenAlignment()
      @_scrollContents.setLocation ps: axis, plus: @newPoint scrollPosition
      @_scrollPositionRelativeToChildrenAlignment = true
      @onNextReady => @_setAtStart()
    else
      if @_scrollPositionRelativeToChildrenAlignment
        log scrollPosition += @getWindowSize()
      @_scrollContents.setAxis point0
      @_scrollContents.setLocation @newPoint scrollPosition
      @_scrollPositionRelativeToChildrenAlignment = false

  _setReferenceFrameIfChanged: (referenceFrame) ->
    if neq referenceFrame, @getReferenceFrame()
      @setReferenceFrame referenceFrame

  _setAtStart: ->
    @_setReferenceFrameIfChanged page: first @getPages()
    @_atStart = true
    @_atEnd = false
    true

  _setAtEnd: ->
    @_setReferenceFrameIfChanged atEndEdge: true, page: last @getPages()
    @_atStart = false
    @_atEnd = true
    true
  ...
  _updateAtStartAndAtEnd: ->
    ...
    if @getContentsFitInWindow()
      ...
      if scrollPosition >= @getMaxScrollPosition()
        @_setAtStart()
      else if scrollPosition <= @getMinScrollPosition()
        @_setAtEnd()
  ...
  _sizeChanged: (newSize, oldSize) ->
    @_updateAtStartAndAtEnd @scrollPosition, newSize
    @_queueUpdateEvent()
    super

  _scrollContentsChildChanged: ->
    unless @_scrollAnimator
      if @contentsFitInWindow || @scrollPosition != bsp = @boundedScrollPosition
        # log scrollPosition: @scrollPosition, boundedScrollPosition: @boundedScrollPosition, totalPageSize: @totalPageSize, windowSize: @getWindowSize()
        @setScrollPosition bsp
    # unless @_scrollAnimator
    #   log setScrollPosition: @getBoundedScrollPosition() , scrollPosition: @getScrollPosition()
    #   @onNextReady => @setScrollPosition @getBoundedScrollPosition()

  _updateHiddenChildren: (scrollMode = @getPendingScroll()) ->
    @_scrollContents ||= new Element
      key: "scrollContents"
      receivePointerEvents: "passToChildren"
      @_pagesBeforeBaselineWrapper = new Element key:"pagesBeforeBaseline", on: sizeChanged: => @_scrollContentsChildChanged()
      @_pagesAfterBaselineWrapper  = new Element key:"pagesAfterBaseline",  on: sizeChanged: => @_scrollContentsChildChanged()
